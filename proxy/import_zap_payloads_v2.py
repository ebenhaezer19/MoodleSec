#!/usr/bin/env python3
"""
Import REAL payloads dari ZAP scan history - FIXED VERSION
(Dengan verbose debugging)
"""
...
class ZAPPayloadImporter:
    ...
    def check_zap(self) -> bool:
        """Check ZAP running"""
        try:
            response = self.client.get(f"{self.zap_base_url}/JSON/core/action/version")
            result = response.status_code == 200
            print(f"[✓] ZAP Version Check: Status {response.status_code}")
    ...
    def get_zap_alerts(self, count: int = 200) -> List[Dict[str, Any]]:
        """Fetch REAL alerts/findings dari ZAP dengan verbose output"""
